...
melee damage, and — on 1.20.5+ — visual size via `Attribute.SCALE`),
a forced equipment loadout with drop chance zeroed out, a custom name, a
glow, and thinned (never boosted) natural spawn frequency. Spawn thinning
(`creatures.spawnMultiplier`) works independently of `creatures.enabled` —
see CHANGES.md v0.1.1 — the rest of this section (reskinning) still needs
`enabled: true`.
