...
**Update (see CHANGES.md v0.1.1):** the two items this list used to flag as
uncertain — `LimitedRegion`'s bounds-check method and the `Attribute`
constant names — were confirmed against a real `mvn package` build against
paper-api 1.21.4 and fixed. `LimitedRegion.isInRegionBounds(int, int, int)`
doesn't exist; the real method is `isInRegion(int, int, int)`, now used
throughout `GiantTreePopulator`/`StructurePopulator`/`DecorationPopulator`.
And every `Attribute.GENERIC_*` constant in `MobCustomizationListener` was
renamed without the `GENERIC_` prefix as part of Minecraft 1.21's attribute
rework (`minecraft:generic.max_health` → `minecraft:max_health`, etc.) —
`MAX_HEALTH`/`MOVEMENT_SPEED`/`ATTACK_DAMAGE`/`SCALE` are the real names now.
Both are fixed in this codebase already; the two list entries below are kept
only as a record of what changed and why, not as things still worth
double-checking.

If anything else fails to compile against the real API, it's most likely one
...
- ~~`LimitedRegion`'s exact bounds-check method name in `GiantTreePopulator`
  ...
  populator-region APIs before across versions).~~ Fixed — see above.
...
- ~~`LimitedRegion.isInRegionBounds` is also used by the new
  ...
  fix if it doesn't resolve.~~ Fixed — see above.
...
- ~~`Attribute.GENERIC_SCALE` in `MobCustomizationListener` (used for
  ...
  project so worth checking first.~~ Fixed — see above. Turned out all
  four `GENERIC_*` constants (`MAX_HEALTH`/`MOVEMENT_SPEED`/
  `ATTACK_DAMAGE`/`SCALE`) needed the prefix dropped for 1.21.x, not just
  the newest one — the "long-stable, not in question" assumption about
  the first three was wrong.
